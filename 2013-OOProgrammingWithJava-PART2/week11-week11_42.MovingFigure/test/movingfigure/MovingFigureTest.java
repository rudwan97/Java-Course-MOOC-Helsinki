...
public class MovingFigureTest {
    ...
    public void setUp() {
        ...
        try {
            laatikkoClass = ReflectionUtils.findClass("movingfigure.movingfigure.Box");
    ...
    public void box() {
        if (laatikkoClass == null || Modifier.isAbstract(laatikkoClass.getModifiers())) {
            fail("Have you created class movingfigure.Box inside the package movingfigure and is the class movingfigure.Box public?");
        }

        if (laatikkoClass.getDeclaredFields().length != 2) {
            fail("Does the class movingfigure.Box have only the instance variables private int width ja private int height? You do not need any other instance variables.");
        }

        if (laatikkoConstructor == null) {
            fail("Does the class movingfigure.Box have constructor public movingfigure.Box(int x, int y, int width, int height)?");
        }

        if (!figureClass.isAssignableFrom(laatikkoClass)) {
            fail("Does the class movingfigure.Box inherit class Figure?");
        }
    ...
    public void compoundFigure() {
        ...
        assertTrue("CompoundFigure kk = new CompoundFigure();\n"
                + "kk.add(new movingfigure.Box(50, 75, 100, 125));\n"
    ...
    private Object luoBox(int x, int y, int leveys, int korkeus) {
        ...
        try {
            ...
            fail("Is the class movingfigure.Box public and is its constructor public?");
        }

        tarkistaSijainti("movingfigure.Box", laatikko, x, y);
        tarkistaSiirtyminen("movingfigure.Box", laatikko, x, y);
        tarkistaFigurenPiirtaminen("new movingfigure.Box(" + x + ", " + y + ", " + leveys + ", " + korkeus + ")", "movingfigure.Box", laatikko, "fillRect(" + x + ", " + y + ", " + leveys + ", " + korkeus + ")");
